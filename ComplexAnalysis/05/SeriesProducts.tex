...
\begin{example}{The Mittag-Leffler Theorem}{The Mittag-Leffler Theorem}
	\begin{itemize}
		...
		\item Conversely, we now study the sum
			...
			So we have
			\begin{equation}
				\frac{\pi}{\sin \pi z} = \frac{1}{z} + \sum_{n=1}^{\infty} (-1)^n \frac{2z}{z^2-n^2} = \limit_{m \to \infty } \sum_{n=-m}^{m} \frac{(-1)^n}{z-n}.
			\end{equation}
	\end{itemize}
\end{example}
...
\begin{theorem}{Infinite Products}{Infinite Products}
	The infinite product $\prod_{n=1}^{\infty} (1 + a_n)$ converges to a nonzero limit iff the series $\sum_{n=1}^{\infty} \log (1 + a_n)$ converges to a limit in the principal branch.
\end{theorem}
...
\begin{remark}
	For uniform convergence we can say the same. We define that an infinite product $\prod_{n=1}^{\infty} (1 + a_n(z))$ converges uniformly in a set $E$ if the series $\sum_{n=1}^{\infty} \log (1 + a_n(z))$ converges uniformly in $E$.
\end{remark}

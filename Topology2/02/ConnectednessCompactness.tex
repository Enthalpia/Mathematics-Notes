...
\begin{definition}{Components}{Components}
	...
	A restatement is that the components of $X$ are maximal connected subspaces of $X$, i.e. disjoint subspaces of $X$ whose union is $X$ and each nonempty connected subspace of $X$ intersects only one of them. They are connected themselves.
\end{definition}
\begin{proof}
	The symmetry and reflexivity is obvious. For transitivity, if $x\sim y$ and $y\sim z$, then there exists connected sets $U,V$ such that $x,y\in U$ and $y,z\in V$. Then $U\cup V$ is connected, as they intersect at $y$. The connectedness of each component is from the union of connected spaces theorem \ref{thm:Union of Connected Subspaces}.
\end{proof}
...
\begin{definition}{Path Components}{Path Components}
	...
	A restatement is that the path components of $X$ are maximal path connected subspaces of $X$. They are path connected themselves.
\end{definition}
\begin{proof}
...
For path connectedness, the analogy of theorem \ref{thm:Union of Connected Subspaces} also holds. This is easy to see, as we can just paste the paths together.

\begin{proposition}{Union of Path-Connected Subspaces}{Union of Path-Connected Subspaces}
	The union of a collection of path connected subspaces of $X$ that has a point in common is path connected, i.e. If $A_{\alpha}\subseteq X$ are path connected, then
	\begin{equation*}
		\bigcap_{\alpha} A_{\alpha} \neq \emptyset \rightarrow \bigcup_{\alpha} A_{\alpha} \text{ is path connected.}
	\end{equation*}
\end{proposition}
...
\begin{remark}
	Local connected means that each point has a connected neighborhood with a arbitrary small scale. The topologist's sine curve fails this, as any neighborhood of a point on the vertical line contains many oscillations, which makes it disconnected.
\end{remark}
...
\begin{theorem}{Relation of Two Connectedness}{Relation of Two Connectedness}
	If $X$ is locally connected, then $X$ is connected iff $X$ is path connected.
\end{theorem}
\begin{proof}
	The components of $X$ are the same as the path components, so if $X$ is connected, then it is path connected, and vice versa.
\end{proof}
...
\begin{definition}{Local Compactness}{Local Compactness}
	A space $X$ is locally compact at $x$ if $\exists U\in \mathcal{T}_X$ such that $\exists C, x\in U \subseteq C$ and $C$ is compact. If $X$ is locally compact at every point, we say that $X$ is \textbf{locally compact}.
